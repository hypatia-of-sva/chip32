...
- Secondly, we need textures. For these, we need again a pair, the first again a pointer to a buffer, but the second a packed two-dimensional size, where the upper 16 bits of the cell indicate the height, the lower 16 bits the width of the texture, both in cells (which represent pixel data in 32-bit RGBA, stored in one cell per pixel). There are 64 texture buffers available, stored in the pairs 0xFFFF0500/01 to 0xFFFF057E/7F
